...
bot = Cinch::Bot.new do
  ...
  on :message, Regexp.new('^\.moe', Regexp::IGNORECASE) do |m|
    #  sleep 2 
    m.reply Cinch::Helpers.sanitize "1200 BTC = $0.00 USD".force_encoding('utf-8')
  end
    ...
    m.message.scan(Regexp.new('(https?://([^\s/]*)\S*)', Regexp::IGNORECASE)) do |url, host|
      ...
      easy.on_body do |chunk, easy|
        ...
        if title_found = $1
          ...
          if m.message !~ Regexp.new('.*(?:youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|watch\?(?:(?!v=)[^&]+&)+v=)([^#\&\?\s]*).*', Regexp::IGNORECASE) &&
            m.message !~ Regexp.new('https?://[^/]*imdb.com.*/title/\D*(\d+)', Regexp::IGNORECASE) &&
            m.message !~ Regexp.new('https?://([^\/\.]*\.)*dumpert\.nl\S+', Regexp::IGNORECASE)
            title_found_reply = "[ " + title_found + " ] - " + host
            m.reply Cinch::Helpers.sanitize coder.decode title_found_reply.force_encoding('utf-8')
          end
        end
      ...
      if m.channel.to_s == '#newzbin'
        begin
          ...
          con.query("INSERT INTO TitleBot(Date, Nick, URL, Title) VALUES (NOW(), '#{con.escape_string(m.user.to_s)}', '#{con.escape_string(url)}', #{!mytitle.nil? ? "'" + con.escape_string(coder.decode mytitle.force_encoding('utf-8')) + "'" : "''"})")
